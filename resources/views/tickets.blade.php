    ...
    <div class="row g-0" style="gap: 25px">
        @foreach(Auth::user()->tickets as $ticket )
            <div class="card col-3" style="width: 18rem;">
                ...
                <div class="card-body">
                    <a href="{{ Route('detail_ticket', ['id' => $ticket->uuid]) }}" class="text-decoration-none">
                        <h5 class="card-title">{{ $ticket->event->display }}</h5>
                    </a>
                    <p class="card-text text-truncate">{{ $ticket->event->description }}</p>
                </div>
            </div>
        @endforeach
    </div>
